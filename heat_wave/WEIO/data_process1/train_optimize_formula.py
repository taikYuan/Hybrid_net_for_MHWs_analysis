...
# 只要表面数据
data1 = np.load(r'D:\heat_wave\WEIO\expand_WEIO\radiation_93_19_expand_WEIO_ocean_last.npz')
print(data1.files)  # ['lat', 'lon', 'slfh', 'sshf', 'ssr', 'str']

slfh = data1['slfh'][:]  # (9861, 65, 49)
sshf = data1['sshf'][:]  # (9861, 65, 49)
ssr = data1['ssr'][:]  # (9861, 65, 49)
str = data1['str'][:]  # (9861, 65, 49)
print(slfh.shape)

lon_radiation = data1['lon'][:]  # 54 54.25 ... 65.5 65.75 66
lat_radiation = data1['lat'][:]  # -8 -7.75 ... 7.75 8


data2 = np.load(r'D:\heat_wave\WEIO\expand_WEIO\SST_93_19_expand_WEIO_area.npz')
print(data2.files)  # ['time', 'lat', 'lon', 'sst']

sst_obser = data2['sst'][:,:65,:] # (9861, 66, 49)

lon_obser = data2['lon'][:] # 54.125, 54.375, ... , 65.875  66.125
lat_obser = data2['lat'][:] # -8.125 -7.875 ..., 7.875  8.125

print(sst_obser.shape)

data3 = np.load(r'D:\heat_wave\WEIO\expand_WEIO\T_d_u_d_v_d_93_19_deep_expand_WEIO_last_chazhi.npz')
print(data3.files) #['T_d', 'u_d', 'v_d']

T_d = data3['T_d'][:] # (9861, 65, 49)
u_d = data3['u_d'][:] # (9861, 65, 49)
v_d = data3['v_d'][:] # (9861, 65, 49)


data4 = np.load(r'D:\heat_wave\WEIO\expand_WEIO\reanalysis_data_1993-2019_expand_WEIO_area.npz')
print(data4.files) #['time', 'lat', 'lon', 'mld', 'salinity', 'temp', 'u', 'v']


mld = data4['mld'][:,:,:]  # (9861, 65, 49)
analysis_temp = data4['temp'][:,0,:,:] # (9861, 65, 49)
u = data4['u'][:,0,:,:] # (9861, 65, 49)
v = data4['v'][:,0,:,:] # (9861, 65, 49)
print(u.shape)
#
#
...
y = np.arange(54, 66.2, 0.25) #(49,)
x = np.arange(-8, 8.2, 0.25)  #(65,)
print(x.shape)
...
print('dT_dx.shape:{}'.format(dT_dx.shape))  # (3281, 7, 41, 80)
...
print('dT_dy.shape:{}'.format(dT_dy.shape))  # (3281, 7, 41, 80)

np.savez(r'D:\heat_wave\WEIO\expand_WEIO\last\optimized_formula_all_variables_expand_WEIO_surface_deepchazhi_1.npz',

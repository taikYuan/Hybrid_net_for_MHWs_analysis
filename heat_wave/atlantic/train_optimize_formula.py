...
# 只要表面数据
data1 = np.load(r'D:\heat_wave\atlantic\radiation_93_19_expand_atlantic_ocean_last.npz')
print(data1.files)  # ['lat', 'lon', 'slfh', 'sshf', 'ssr', 'str']

slfh = data1['slfh'][:]  # (9861, 81, 81)
sshf = data1['sshf'][:]  # (9861, 81, 81)
ssr = data1['ssr'][:]  # (9861, 81, 81)
str = data1['str'][:]  # (9861, 81, 81)

lon_radiation = data1['lon'][:]  # -30 -29.75  -29.5 ... -10.5  -10.25 -10  ---> 330 330.25 330.5 ... 349.5  349.75  350
lat_radiation = data1['lat'][:]  # -30 -29.75  -29.5 ... -10.5  -10.25 -10

data2 = np.load(r'D:\heat_wave\atlantic\SST_93_19_expand_atlantic_area.npz')
print(data2.files)  # ['time', 'lat', 'lon', 'sst']

sst_obser = data2['sst'][:,:81,:81] # (9861, 82, 82)

lon_obser = data2['lon'][:] # 329.875 330.125 ... 349.875 350.125
lat_obser = data2['lat'][:] # -30.125 -29.875 -29.625 ... 10.375 10.125 -9.875

data3 = np.load(r'D:\heat_wave\atlantic\T_d_u_d_v_d_93_19_deep_expand_atlantic_last.npz')
print(data3.files) #['T_d', 'u_d', 'v_d']

T_d = data3['T_d'][:] # (9861, 81, 81)
u_d = data3['u_d'][:] # (9861, 81, 81)
v_d = data3['v_d'][:] #(9861, 81, 81)

data4 = np.load(r'D:\heat_wave\atlantic\reanalysis_data_1993-2019_expand_atlantic_area.npz')
print(data4.files) #['time', 'lat', 'lon', 'mld', 'salinity', 'temp', 'u', 'v']


mld = data4['mld'][:,:,:]  # (9861, 40, 81)
analysis_temp = data4['temp'][:,0,:,:] # (9861, 30, 81, 81)
...
y = np.arange(330, 350.2, 0.25) #(81,)
x = np.arange(-30, -9.8, 0.25)  #(81,)
...
np.savez(r'D:\heat_wave\atlantic\train_last_data\optimized_formula_all_variables_expand_atlantic_surface2.npz',
